...
- **Carry the two-stage build and release tooling** — Linux-hosted
  cross-compilation of `zed` and `cli` for macOS (the r740 box by hand, CNB
  automatically), a Mac-side bundle step, a verifier, an install/rollback
  switch and CNB release publishing. Remote servers come from upstream's
  releases. BUILDING.md is the guide; §3.7 lists the tracked
  `script/bundle-mac` modifications.
...
cross-compiles `zed` and `cli` for `aarch64-apple-darwin` inside the
container image from `.cnb/Dockerfile.zed-macos`, on the r740 Linux box by
hand (`script/cross-build-remote`, about 12 minutes warm) and on CNB
automatically for every `enhanced/*` tag (`.cnb.yml`, Dev-CPU pool, 16 cpu).
Remote servers are not built: the app fetches upstream's own for the same
version and channel (BUILDING.md §0). **Stage 2** on the Mac assembles and ad-hoc
...
| Date       | From          | To             | Notes                                                                                                |
| ---------- | ------------- | -------------- | ---------------------------------------------------------------------------------------------------- |
| 2026-09-14 | `v1.20.0-pre` | `v1.20.0-pre`  | Fork-only restructuring, no baseline change. Rollback tag `archive/enhanced/v1.20.0-pre-20260914-122557`. Rebuilt the 15-commit stack as the 8-commit, three-group shape of §3 (features 1–5 incl. `agent_ui: Surface models…`, build shims 6, tooling 7, docs 8), tree-identical to the old tip except for the deleted `.github/workflows/build-enhanced.yml`; upstream's `release.yml` and the stale enhanced workflows disabled in the fork's Actions settings; releases move to CNB (§11). Tooling: `.cnb.yml` rewritten as a Dev-CPU workspace build plus a 1-cpu `tag_push` trigger, compiler-rt built from LLVM source in the image, `aarch64-unknown-linux-musl` and the `cnb` CLI added, `script/install-mac-app`, `script/release-mac-assets`, `script/publish-cnb-release`, `ZED_UPDATE_EXPLANATION` compiled in; BUILDING.md split out of this file, mermaid charts in both. Validated: r740 stage 1 of `9674eeb2` in 1,093 s after a 10-min image rebuild, stage 2 44 s, `verify-mac-app` clean incl. the crash-handler quit, `install-mac-app` switched the daily Preview to `~/Applications` (previous kept in `/Applications`, db backed up); CNB branch build of the same tree in 58 min on 64 cpu (Darwin link with no Apple compiler-rt, both Linux musl remote servers) after three short failures recorded in BUILDING.md §8.2. Published as re-spin `enhanced/v1.20.0-pre.2`: Mac assets from the r740 build of the tag commit (stage 1 726 s, stage 2 46 s) uploaded with `publish-cnb-release`; CNB stage-1 assets from a workspace on the `enhanced` tip started by commit sha. The tag push did run the trigger, which then failed on `workspace/start`: the API resolves `branch` against branches only and rejects a slash-containing tag in `ref`, so the trigger now clears the branch's workspace and starts by `CNB_COMMIT`, and the publish stage resolves the tag from the commit (BUILDING.md §2.2). Two more CNB runs then fixed `publish-cnb-release` for the Linux image (REST API instead of the version-dependent `cnb` CLI; `wc -c` instead of BSD `stat`) before the stage-1 assets landed: the release holds all 12 files (app tarball, DMG, three remote servers with checksums, Darwin package with checksum and provenance). Evening follow-up: the CNB runner was resized from 64 to 16 cpu after the stage logs showed 4 to 7 cores in use against a 23 GiB link peak, a web-button trigger was added, and the day's `fixup!` commits were autosquashed back into the eight-commit shape; `enhanced/v1.20.0-pre.2` keeps pointing at its original commit, which is no longer on the branch. Then, on the operator's decision, stage 1 stopped building the three remote servers (the fork does not touch them and the app downloads upstream's for its version and channel); the pipeline lost zig and about 36 minutes, a 16-cpu cold compile measured 14:42 at 3.4 cores, and the publish stage became first-publisher-wins. Final shape validated on both hosts: r740 stage 1 449 s, stage 2 42 s; CNB 22 min cold at 16 cpu, about 6 core-hours. Dockerfile made reproducible: Debian base by digest, toolchain packages by exact Debian version, rustup and toolchain by version and checksum, cargo-bundle by commit, zig and the CNB CLI dropped, toolchain identity recorded in `.build.json`. |
...
| CNB workspaces are keyed by branch and start on a commit, never on a tag name; the first tag-triggered build failed on that. | Automation that hands a ref to another system must pass the commit sha and let the receiver re-derive the tag. | The trigger passes `CNB_COMMIT`; the publish stage finds the `enhanced/*` tag whose sources equal `HEAD` (BUILDING.md §2.2). |
| A 64-cpu CNB runner averaged 4 to 7 busy cores; the first sizing was a guess. | Size runners from measured utilization (`/usr/bin/time -v` is already in every stage log) and from the memory peak, not from the host's core count. | 16 cpu / 32 GiB, the memory floor of the `zed` link (BUILDING.md §1.3). |
| The two hosts built the cross-build image from the same Dockerfile hours apart and got different `cnb` CLI versions; one of them broke the release upload. | An image that both hosts must agree on has to pin every input, and the build must record what it got. | Every `ARG` in `.cnb/Dockerfile.zed-macos` is a pin; `/etc/zed-yolo-toolchain.json` travels in `.build.json` (BUILDING.md §1.2). |
