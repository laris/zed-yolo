...
# `zed` and `cli` Mach-O binaries inside the CNB cross-build container on any
# x86_64 Linux host that has Docker. Leaves
...
# Only the desktop binaries are built: the app downloads upstream's official
# zed-remote-server assets for the same version and channel, and the fork
# does not change the remote server (decision 2026-09-14, BUILDING.md §0).
timed "build zed+cli" docker_run bash .cnb/run-zed-build-step.sh build "$target" zed-cli --package zed --package cli
timed package docker_run bash .cnb/run-zed-build-step.sh package "$target" zed-cli
